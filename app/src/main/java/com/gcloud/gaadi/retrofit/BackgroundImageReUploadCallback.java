 ...
 */
public class BackgroundImageReUploadCallback implements Callback<ImageUploadResponse> {

    final String TAG = "BackgroundImageReUploadCallback";

    private Context context;
    private FinanceData financeData;
    private FinanceDB financeDB;

    public BackgroundImageReUploadCallback(Context context,
    ...
    public void success(ImageUploadResponse imageUploadResponse, Response response) {
        updateCount(RetrofitImageReUploadService.KEY_TOTALCOUNT);
        ...
        if (imageUploadResponse.getStatus().equals("T")) {
            if (financeDB.updateSuccess(financeData.getId()) != 1)
                updateCount(RetrofitImageReUploadService.KEY_FAILCOUNT);
            else {
...
//        updateNotification();
    ...
    private void checkForRetry() {
        String message = "";
        int maxCount = CommonUtils.getIntSharedPreference(context, RetrofitImageReUploadService.KEY_MAXCOUNT, 0),
                totalCount = CommonUtils.getIntSharedPreference(context, RetrofitImageReUploadService.KEY_TOTALCOUNT, 0),
                failCount = CommonUtils.getIntSharedPreference(context, RetrofitImageReUploadService.KEY_FAILCOUNT, 0);
        if (totalCount == maxCount) {
//            GCLog.e(TAG, "All images Uploaded");
            if (failCount > 0) {
//                GCLog.e(TAG, "failCount: " + failCount);
                int maxRetry = CommonUtils.getIntSharedPreference(context, RetrofitImageReUploadService.KEY_MAXRETRY, 0);
                if (maxRetry >= 0 && ApplicationController.checkInternetConnectivity()) {
                    maxRetry--;
                    CommonUtils.setIntSharedPreference(context, RetrofitImageReUploadService.KEY_MAXRETRY, maxRetry);
                    ApplicationController.getInstance().startService(
                            new Intent(ApplicationController.getInstance(), RetrofitImageReUploadService.class));
                } else {
