...
public class WarrantyFilter extends AppCompatActivity implements View.OnClickListener {

    public static final String CHANGE_LIST_ITEM = "changeListItem";
    final public static ArrayList<String> list = new ArrayList<>(
            Arrays.asList(new String[]{"Reg No", "Make", "Model", "Warranty Status", "Warranty Type"}));
            ...
            case "":
                ...
                if (!currentKey.isEmpty()
                    ...
                    if (StocksActivity.filterParams.get(key).contains(dataKey)) {
                        ...
                        if ("warranty status".equalsIgnoreCase(key)) {
                            StocksActivity.filterParams.get(key).clear();
                            ((FilterFragment)
                                    ((FilterOptionPagerAdapter)
                                            filterOptionViewPager.getAdapter()).getItem(list.indexOf("Warranty Status")))
                                    .clearSelection(dataKey);
                        }
